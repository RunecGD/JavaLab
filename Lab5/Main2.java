...
public class Main2 {
    public static void main(String[] args) {
        ...
        Date[] dates = new Date[4]; // Массив для хранения 4 дат

        // Заполнение массива объектами Date
        for (int i = 0; i < dates.length; i++) {
            dates[i] = inputDate(scanner); // Вводим дату через отдельный метод
        }
        ...
        // Увеличиваем каждую дату на 5 дней и выводим
        for (int i = 0; i < dates.length; i++) {
            dates[i].addDays(5);
    ...
    private static Date inputDate(Scanner scanner) {
        Date date = new Date(); // Создаем объект Date
        while (true) {
            try {
                ...
                return date; // Возвращаем созданную дату
